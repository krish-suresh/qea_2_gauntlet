function [bestInlierSet,bestOutlierSet,bestEndPoints, hasLine] = ransacLine(points,lineTol, lineIterations, lineMaxGap)    
     bestcandidates = [];
    ...
    for k=1:lineIterations
        ...
        if biggestGap < lineMaxGap  && sum(inliers) > size(bestInlierSet,1) && pdist(endPoints,'euclidean') > 0.4
